...
for i=1:days
    ...
    error_mean(i)=mean((abs(opr_multi(i).Ym-rot90(concal(:,1)',2))./opr_multi(i).Ym));%绝对误差
    error_sum(i)=sum(abs(opr_multi(i).Ym-rot90(concal(:,1)',2)));%相对误差
    disp(['第',num2str(i),'组  模型测试结果                炼厂产品数据']);
    disp(['轻组分质量含量为：',num2str(concal(1,1)),'%            ',num2str(opr_multi(i).Ym(6)),'%']);
    disp(['轻石质量含量为：',num2str(concal(2,1)),'%              ',num2str(opr_multi(i).Ym(5)),'%']);
    disp(['重石质量含量为：',num2str(concal(3,1)),'%              ',num2str(opr_multi(i).Ym(4)),'%']);
    disp(['航煤质量含量为：',num2str(concal(4,1)),'%             ',num2str(opr_multi(i).Ym(3)),'%']);
    disp(['柴油质量含量为：',num2str(concal(5,1)),'%              ',num2str(opr_multi(i).Ym(2)),'%      第',num2str(i),'组原油数据的相对误差为：',num2str(100*error_mean(i)),'%.']);
    disp(['尾油质量含量为：',num2str(concal(6,1)),'%             ',num2str(opr_multi(i).Ym(1)),'%       第',num2str(i),'组原油数据的绝对误差之和为：',num2str(error_sum(i)),'%.']);
